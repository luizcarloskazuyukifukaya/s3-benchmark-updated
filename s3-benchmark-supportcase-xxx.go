// s3-benchmark-support-xxx.go
// Copyright (c) 2017 Wasabi Technology, Inc.
// October 23, 2024
// Updated for better logging and data analytics (Kazuyuki Fukaya)
// ADD: Updated to emulate a support case
//
//	Only perform Upload, NO DOWNLOAD NOR DELETE
package main
...
// FOR SUPPORT CASE XXX
func runOnceUpload(thread_num int) {
	// No for loop based on the duration of time
	objnum := atomic.AddInt32(&upload_count, 1)
	fileobj := bytes.NewReader(object_data)
	prefix := fmt.Sprintf("%s/%s/Object-%d", url_host, bucket, objnum)
	req, _ := http.NewRequest("PUT", prefix, fileobj)
